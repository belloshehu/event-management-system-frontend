...
export default function AdminPartnershipPage() {
  ...
  const { isLoading, data } = useGetUsers({ filter: { role: "partner" } });

  if (!session) router.push("/login");

  return (
    <>
      {isMobile && data?.length && <SearchInput placeholder="Search for partners" />}
      <Title title="Partners" className="mb-5 bg-green-100 p-2" />
      <UserList
